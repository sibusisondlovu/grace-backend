...
router.post('/ai-assistant', async (req, res) => {
    try {
        ...
        if (lastMessage.toLowerCase().includes('document') || lastMessage.toLowerCase().includes('report')) {
         ...
         LIMIT 5`, [committeeIds.length > 0 ? committeeIds : ['00000000-0000-0000-0000-000000000000']]);
            if (documentsResult.rows.length > 0) {
                context += "\n\nRecent Documents:\n" + documentsResult.rows.map(d => `- ${d.title} (${d.document_type}) from ${d.committee_name || 'Unknown Committee'} on ${new Date(d.meeting_date).toLocaleDateString()}`).join('\n');
            }
        }
        ...
        if (lastMessage.toLowerCase().includes('action') || lastMessage.toLowerCase().includes('task')) {
         ...
         LIMIT 5`, [committeeIds.length > 0 ? committeeIds : ['00000000-0000-0000-0000-000000000000']]);
            if (actionsResult.rows.length > 0) {
                context += "\n\nRecent Action Items:\n" + actionsResult.rows.map(a => `- ${a.title} (${a.status}, ${a.priority} priority) - Due: ${a.due_date ? new Date(a.due_date).toLocaleDateString() : 'Not set'} - Committee: ${a.committee_name || 'Unknown'}`).join('\n');
            }
        }
        // Search for meetings
        if (lastMessage.toLowerCase().includes('meeting') || lastMessage.toLowerCase().includes('agenda')) {
         ...
         LIMIT 5`, [committeeIds.length > 0 ? committeeIds : ['00000000-0000-0000-0000-000000000000']]);
            if (meetingsResult.rows.length > 0) {
                context += "\n\nRecent Meetings:\n" + meetingsResult.rows.map(m => `- ${m.title} (${m.status}) on ${new Date(m.meeting_date).toLocaleDateString()} - Committee: ${m.committee_name || 'Unknown'}`).join('\n');
            }
        }
        ...
        if (lastMessage.toLowerCase().includes('resolution') || lastMessage.toLowerCase().includes('motion')) {
         ...
         LIMIT 5`, [committeeIds.length > 0 ? committeeIds : ['00000000-0000-0000-0000-000000000000']]);
            if (motionsResult.rows.length > 0) {
                context += "\n\nRecent Motions/Resolutions:\n" + motionsResult.rows.map(m => `- ${m.title} (${m.motion_type}, ${m.status}) - Outcome: ${m.outcome || 'Pending'} - Committee: ${m.committee_name || 'Unknown'}`).join('\n');
            }
        }
...
router.post('/send-notification-email', async (req, res) => {
    try {
        const { organizationId, recipientEmail, recipientId, subject, title, message, notificationType, metadata = {}, } = req.body;
